# Rescources related to light sync with music in Rust

## Working audio projects in Rust

* [Implementation of loopback audio capture](https://github.com/dheijl/swyh-rs/blob/master/src/utils/audiodevices.rs)

## Audio processing

* [Digital audio processing crate](https://crates.io/crates/dasp)
* [Digital audio concepts](https://developer.mozilla.org/en-US/docs/Web/Media/Formats/Audio_concepts)

### Onset detection

* [A tutorial on onset detection in music signals](https://ieeexplore.ieee.org/abstract/document/1495485)
* [State of the art real time onset detection](https://www.dafx12.york.ac.uk/papers/dafx12_submission_4.pdf)

## Light control

* [DMX Crate](https://docs.rs/dmx/latest/dmx/)
* [WLED API](https://kno.wled.ge/interfaces/json-api/)

## Embedded

* [Embedded Rust book](https://docs.rust-embedded.org/discovery/microbit/)
* [Presentation about bare metal Rust audio programming](https://flowdsp.io/talks/talk-adc20/#1)

## Webaudio

* [Web Audio Rust API](https://rustwasm.github.io/wasm-bindgen/examples/web-audio.html)
* [Web Audio Reference](https://developer.mozilla.org/en-US/docs/Web/API/Web_Audio_API)
* [CPAL WASM Guide](https://github.com/RustAudio/cpal/wiki/Setting-up-a-new-CPAL-WASM-project)
