...
use super::{window, threshold::AdvancedThreshold};
...
impl SpecFlux {
    pub fn init() -> SpecFlux{
        let bank = vec![0.0; FREQUENCIES.len() - 2];
        let threshold = AdvancedThreshold::init();

        SpecFlux { bank, threshold }
    }
    ...
    pub fn detect(
        ...
        let bank = (0..FREQUENCIES.len() - 2)
            .map(
                |b| {
                    let range = FREQUENCIES[b] as usize .. FREQUENCIES[b + 2] as usize;
                    let window = window(range.len(), super::WindowType::Triangular);
                    (freq_bins[range]
                        .iter()
                        .zip(&window)
                        .map(|(&f, &w)| f * w)
        ...
        let weight: f32 = old_bank.iter().zip(&bank).map(|(&a, &b) | (((b - a) + (b - a).abs()) / 2.0)).sum();
...
pub fn spec_flux(freq_bins: &Vec<f32>) {
    let mut bank = vec![0.0; FREQUENCIES.len() - 4];
    
    bank.iter_mut()
        .enumerate()
        .for_each(
            |(b, x)| {
                let range = FREQUENCIES[b] as usize .. FREQUENCIES[b + 4] as usize;
                let window = window(range.len(), super::WindowType::Triangular);
                *x = freq_bins[range].iter().zip(&window).map(|(&f, &w)| f * w).sum();
            }
        );
}