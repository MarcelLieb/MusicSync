use std::{sync::{Mutex, Arc, mpsc::{Sender, TryRecvError}}, thread};
use std::sync::mpsc;
use std::marker::Send;
...
impl<T: Stream + Send + Sync + 'static> PollingHelper<T> {
    pub fn init<F>(stream: Arc<T>, formatter: Arc<dyn Fn(&[[u16; 3]]) -> Vec<u8> + Send + Sync>, polling_frequency: u16)  -> PollingHelper<Arc<T>>
    where 
        Arc<T>: Stream
    {
        ...
        thread::spawn(move || {
            loop {
                ...
                drop(colors);
                thread::sleep(std::time::Duration::from_millis((1000 / polling_frequency) as u64));
            }
        });
        return PollingHelper {colors, polling_frequency, poller, tx};
    }
    ...
    pub fn update_color(&mut self, colors: &[[u16; 3]], additive: bool) {
        ...
        if additive {
            colors_lock.iter_mut().zip(colors.iter()).for_each(|(old, new)| {
